...
typedef struct dma_proxy_channel_interface DMAProxyChannelInterface;

static DMAProxyChannelInterface* tx_proxy_interface_p;
static int tx_proxy_fd;
static int test_size;
static int test_count = 1;

int test(char* input_message, int message_size);
...
int test(char* input_message, int message_size){
    DMAProxyChannelInterface* rx_proxy_interface_p;
    int rx_proxy_fd, i;
    int dummy;
    int counter;
    pthread_t tid;
    ...
    /* Map the transmit and receive channels memory into user space so it's accessible
 	 */
    tx_proxy_interface_p = (DMAProxyChannelInterface*)mmap(NULL, sizeof(DMAProxyChannelInterface),
        PROT_READ | PROT_WRITE, MAP_SHARED, tx_proxy_fd, 0);

    rx_proxy_interface_p = (DMAProxyChannelInterface*)mmap(NULL, sizeof(DMAProxyChannelInterface),
    ...
    for (counter = 0; counter < test_count; counter++) {
        ...
        for (i = 0; i < test_size; i++) 
        {
            ...
            if (rx_proxy_interface_p->buffer[i] != tx_proxy_interface_p->buffer[i]) {
                    printf("buffer not equal, index = %d, data = %c expected data = %c\n", i,
                        rx_proxy_interface_p->buffer[i], 
                        tx_proxy_interface_p->buffer[i]);
            }
        } 
        printf("Output buffer: %s\n", rx_proxy_interface_p->buffer);
    }
    pthread_join(tid, NULL);

    /* Unmap the proxy channel interface memory and close the device files before leaving
	 */
    munmap(tx_proxy_interface_p, sizeof(DMAProxyChannelInterface));
    munmap(rx_proxy_interface_p, sizeof(DMAProxyChannelInterface));
