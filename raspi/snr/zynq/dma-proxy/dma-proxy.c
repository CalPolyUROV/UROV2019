...
static void transfer(struct dma_proxy_channel* pchannel_p)
{
	 ...
	 */
    pchannel_p->dma_handle = (dma_addr_t)(pchannel_p->interface_phys_addr + offsetof(struct dma_proxy_channel_interface, buffer));
...
static void test(void)
{
	 ...
	 */
    for (i = 0; i < TEST_SIZE; i++)
        if (channels[TX_CHANNEL].interface_p->buffer[i] != channels[RX_CHANNEL].interface_p->buffer[i]) {
